...
E.3.5 Determine the true power factor at each applicable load point specified in Table 1 of this appendix for each DPPP variety and speed configuration as a ratio of driver power input to the motor (or controls, if present) (*P*_i_), in watts, divided by the product of the voltage in volts and the current in amps at each load point *i*, as shown in the following equation:
...
3 in accordance with the following equation:
