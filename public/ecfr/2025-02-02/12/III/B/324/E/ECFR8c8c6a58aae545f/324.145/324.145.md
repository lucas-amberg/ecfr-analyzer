...
(i) SE* equals max {0, [SE − C × (1− H_s_ − H_fx_)]};
...
(iv) H_s_ equals the haircut appropriate to the collateral type; and


(v) H_fx_ equals the haircut appropriate for any currency mismatch between the collateral and the exposure.
...
(i) An FDIC-supervised institution must use the collateral type haircuts (H_s_) in Table 1 to § 324.132 of this subpart;


(ii) An FDIC-supervised institution must use a currency mismatch haircut (H_fx_) of 8 percent if the exposure and the collateral are denominated in different currencies;
