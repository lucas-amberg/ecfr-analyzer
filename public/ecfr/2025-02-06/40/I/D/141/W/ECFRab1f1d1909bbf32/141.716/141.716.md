...
(b) *Alternative source.* (1) A system may conduct source water monitoring that reflects a different intake location (either in the same source or for an alternate source) or a different procedure for the timing or level of withdrawal from the source (alternative source monitoring). If the State approves, a system may determine its bin classification under § 141.710 based on the alternative source monitoring results.
