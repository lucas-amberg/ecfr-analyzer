...
This section describes coastdown procedures that are unique to vehicles above 14,000 pounds GVWR. These procedures are valid for calculating road-load coefficients for chassis and post-transmission powerpack testing. These procedures are also valid for calculating drag area (*C*_d_*A*) to demonstrate compliance with Phase 1 greenhouse gas emission standards under 40 CFR part 1037.
...
(ii) Determine drag area, *C*_d_*A,* as follows instead of using the procedure specified in Section 10 of SAE J1263:
...
(B) Calculate the vehicle's effective mass, *M*_e,_ in kg by adding 56.7 kg to the measured vehicle mass, *M,* for each tire making road contact. This accounts for the rotational inertia of the wheels and tires.


(C) Calculate the road-load force for each measurement interval, *F*_i_, using the following equation:


(D) Plot the data from all the coastdown runs on a single plot of *F*_i_ vs. *v*_i_2 to determine the slope correlation, *D,* based on the following equation:


(E) Calculate drag area, *C*_d_*A,* in m
...
[N] [79 FR 23823, Apr. 28, 2016, as amended at 81 FR 74202, Oct. 25, 2016; 89 FR 28211, Apr. 18, 2024]
